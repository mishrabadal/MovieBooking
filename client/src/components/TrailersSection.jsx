import React, { useState } from 'react'
 ...
 import { ClockIcon,CalendarIcon,ArrowRight, PlayCircleIcon } from 'lucide-react'
import { useNavigate } from 'react-router-dom'
import BlurCircle from './BlurCircle'
import ReactPlayer from 'react-player'

const TrailersSection = () => {
    const [currentTrailer, setCurretTrailer]=useState(dummyTrailers[0])
...
<div className='group grid grid-cols-4 gap-4 md:gap-8 mt-8 max-w-3xl mx-auto'>
  {dummyTrailers.map((trailer) => (
    <div key={trailer.image} className='relative group-hover:not-hover:opacity-50 hover:-translate-y-1 duration-300 transition max-md:h-60 md:max-h-60 cursor-pointer' 
      onClick={() => setCurretTrailer(trailer)}>
